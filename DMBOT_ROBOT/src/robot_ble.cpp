...
static inline void rs485_reportRelayStateOnce(byte relayState) {
  ...
  g_lastReportedRelay = relayState;
  rs485_reportRelayState(relayState);
}
...
static inline void forceRelayOffAndReport(const char* reason) {
  ...
  if (getRelayState()) {
    ...
    // 이미 OFF여도, 보고는 “변화 있을 때만”이므로 과다 전송 없음
    Serial.print("[FORCE OFF] already off | ");
    Serial.println(reason ? reason : "");
  }

  rs485_reportRelayStateOnce(0);
}

void hardReset(const char* reason) {
  forceRelayOffAndReport("hardReset()");
  delay(30);
  Serial.print(">>> HARD RESET: ");
  Serial.println(reason ? reason : "(no reason)");
  delay(20);
...
static void onAuthFailure(const char* reason) {
  ...
  // 인증 실패도 안전이 우선: OFF + 0 보고
  forceRelayOffAndReport("auth failure");
...
void ble_init() {
  for (int i = 0; i < 5; i++) {
    if (BLE.begin()) {
      ...
      // 초기에도 안전: 릴레이 OFF + 0 보고(변화 있을 때만)
      forceRelayOffAndReport("ble_init()");
      return;
    }
    Serial.println("BLE init failed - retrying...");
    delay(200);
...
void ble_run() {
  ...
  if (robotState == SCANNING) {
    ...
    if (!peripheral.connected()) {
      ...
      forceRelayOffAndReport("BLE disconnected");
    ...
    if (now - lastRSSILog >= 1000) {
      ...
      if (rssiFilt <= CONNECTED_RSSI_BAD_DBM) {
        ...
        if (connRssiBadStreak >= CONNECTED_RSSI_BAD_CONSEC) {
          ...
          forceRelayOffAndReport("RSSI weak -> disconnect");
    ...
    if (currentMillis - lastReportTime >= 5000) {
      ...
      if (peripheral.connected() && robotRelayChar && robotRelayChar.canWrite()) {
        if (!robotRelayChar.writeValue((uint8_t)relayState)) {
          ...
          forceRelayOffAndReport("BLE write fail");
        ...
        forceRelayOffAndReport("robotRelayChar invalid");
