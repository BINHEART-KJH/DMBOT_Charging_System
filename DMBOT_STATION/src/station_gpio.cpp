...
static const float alpha = 0.03f;

// ======================= 분압/보정 =======================
static const float R_TOP = 270000.0f;
static const float R_BOT =  30000.0f;
static const float VIN_SCALE = (R_TOP + R_BOT) / R_BOT; // = 10.0

static const float ADC_VREF_V = 3.27f;
static const float VIN_OFFSET = 0.04f;

// ======================= (참고용) 전압 조건 =======================
// 지금은 릴레이 제어에 사용하지 않음(주석 처리)
static const float DISCONNECT_V          = 6.0f;
static const float CHARGE_START_MIN_V    = 19.0f;
static const float CHARGE_START_MAX_V    = 27.5f;
static const float CHARGE_STOP_V         = 29.5f;
...
#define ENABLE_RS485 1
  ...
  static void rs485ParseStream() {
    while (true) {
      ...
      String one = rs485RxStream.substring(0, ed + 3);
  ...
  // RS485 프레임 처리: ACK만 주고, 릴레이 제어는 "무시"
  static void processRs485Frame(const String& frame) {
    ...
    if (label == "BMS_ROBOT_CTRL_BAT_ON" || label == "BMS_STATION_CTRL_BAT_ON") {
      Serial.print("[RS485] CMD ignored (BLE-only relay). label=");
      Serial.print(label);
      Serial.print(" value=");
...
// ======================= ADC 읽기(모니터링용) =======================
static float getFilteredVoltage()
{
  int raw = analogRead(ADC_PIN);
...
#if ENABLE_RS485
  // 필요하면 상태 프레임만 송신(명령은 무시하지만, 상태 공유는 가능)
  rs485SendStatusFrame("BMS_STATION_BAT_ON", on ? 1 : 0);
#endif
}
